            ...
            {transactions.length === 0 ? (
              ...
              transactions.map((tx) => {
                ...
                const typeIcon = {
                  ...
                  income: <ArrowDownLeft className="w-4 h-4 text-emerald-500" />,
                  purchase: <ArrowUpRight className="w-4 h-4 text-rose-500" />
                }[tx.type];

                const badgeBg = {
                  topup: 'bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400',
                  income: 'bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400',
                  purchase: 'bg-rose-500/10 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400'
                }[tx.type];
                  ...
                  <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-foreground truncate max-w-[150px] sm:max-w-xs">{tx.description}</p>
                    ...
                    {/* Amount */}
                    <td className={`p-4 text-right font-extrabold text-sm tracking-wide ${isDebit ? 'text-foreground' : 'text-emerald-600 dark:text-emerald-400'}`}>
