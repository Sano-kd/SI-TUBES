...
import { User, Mail, Phone, LogOut, ShieldAlert, HelpCircle, Save, Edit3, Settings, ChevronRight, FileText, MessageCircleQuestion } from 'lucide-react';
    ...
    <div className="space-y-6 pb-12">
      ...
      <Dialog isOpen={editDialogOpen} onClose={() => setEditDialogOpen(false)} title="Edit Profil Pengguna">
        <form onSubmit={handleEditSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-muted-foreground block mb-1.5">Nama Lengkap</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="text-xs font-bold text-muted-foreground block mb-1.5">Nomor Kontak (WhatsApp)</label>
            <input
              type="tel"
              value={contact}
          ...
          <div>
            <label className="text-xs font-bold text-muted-foreground block mb-1.5">URL Foto Avatar (Opsional)</label>
            <input
              type="text"
              value={avatar}
      ...
      {/* Help Center Dialog */}
      <Dialog isOpen={helpDialogOpen} onClose={() => setHelpDialogOpen(false)} title="Pusat Bantuan">
        <div className="space-y-4 text-xs">
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="p-3.5 bg-muted/40 rounded-2xl border border-border/40 space-y-1.5">
          ...
          <p className="text-center text-muted-foreground pt-2">
            Masih ada pertanyaan? Hubungi admin melalui email: <span className="text-primary font-bold">support@ekantin.id</span>
          </p>
        </div>
      </Dialog>

      {/* Privacy Policy Dialog */}
      <Dialog isOpen={privacyDialogOpen} onClose={() => setPrivacyDialogOpen(false)} title="Kebijakan Privasi">
        <div className="space-y-4 text-xs text-muted-foreground max-h-96 overflow-y-auto">
          <div className="space-y-4">
            <div>
              <h5 className="font-bold text-foreground mb-1">1. Pengumpulan Data</h5>
              <p className="leading-relaxed">e-Kantin Digital mengumpulkan data pribadi yang Anda berikan saat mendaftar, termasuk nama, email, dan nomor kontak. Data ini digunakan semata-mata untuk keperluan layanan pemesanan makanan di lingkungan kampus.</p>
            </div>
            <div>
              <h5 className="font-bold text-foreground mb-1">2. Penggunaan Data</h5>
              <p className="leading-relaxed">Data Anda digunakan untuk: memproses pesanan, mengelola saldo dompet digital, mengirimkan notifikasi status pesanan, dan meningkatkan kualitas layanan.</p>
            </div>
            <div>
              <h5 className="font-bold text-foreground mb-1">3. Keamanan Data</h5>
              <p className="leading-relaxed">Kami berkomitmen menjaga keamanan data Anda. Seluruh transaksi dienkripsi dan tidak dibagikan kepada pihak ketiga tanpa persetujuan Anda.</p>
            </div>
            <div>
              <h5 className="font-bold text-foreground mb-1">4. Penyimpanan Data</h5>
              <p className="leading-relaxed">Data disimpan secara lokal dalam browser Anda (localStorage) sebagai simulasi. Pada implementasi nyata, data akan disimpan di server yang aman dengan enkripsi standar industri.</p>
            </div>
            <div>
              <h5 className="font-bold text-foreground mb-1">5. Hak Pengguna</h5>
              <p className="leading-relaxed">Anda berhak mengakses, memperbarui, atau menghapus data pribadi Anda kapan saja melalui halaman Profil atau dengan menghubungi tim dukungan kami.</p>
            </div>
            <div>
              <h5 className="font-bold text-foreground mb-1">6. Perubahan Kebijakan</h5>
              <p className="leading-relaxed">Kebijakan ini dapat diperbarui sewaktu-waktu. Perubahan signifikan akan diberitahukan melalui notifikasi aplikasi.</p>
            </div>
          </div>
          <p className="text-center pt-2 border-t border-border">Terakhir diperbarui: Juni 2026 • e-Kantin Digital v1.0</p>
        </div>
      </Dialog>
    </div>
  );
