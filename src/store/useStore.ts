...
interface AppState {
  ...
  register: (
    details: Omit<User, "id" | "balance"> & { passwordConfirm: string },
  ) => { success: boolean; message: string };
  logout: () => void;
...
const INITIAL_PRODUCTS: Product[] = [
  // Kantin Mencintai (usr-2) - Foods
  {
    ...
    description:
      "Nasi hangat dengan ayam pop corn gurih dan telur mata sapi setengah matang disiram saus mentai.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
  ...
  },
  {
    ...
    rating: 4.7,
    image:
      "https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=600&auto=format&fit=crop&q=60",
    description:
      "Nasi goreng bumbu rempah dengan topping bakso, sosis, telur ayam orak-arik, dan sayuran segar.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
  ...
  },
  {
    ...
    rating: 4.9,
    image:
      "https://images.unsplash.com/photo-1562967914-608f82629710?w=600&auto=format&fit=crop&q=60",
    description:
      "Dada ayam krispi digeprek dengan sambal bawang khas solo, diselimuti keju mozzarella leleh hangat.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
  ...
  },
  {
    ...
    description:
      "Mie goreng lezat disajikan dengan chicken katsu renyah dan sambal cabe ijo pedas mantap.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
  ...
  // Kantin Mencintai - Drinks
  {
    ...
    rating: 4.9,
    image:
      "https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=600&auto=format&fit=crop&q=60",
    description:
      "Kopi blend es premium dengan sirup karamel manis lembut, whipped cream, dan saus karamel di atasnya.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
      ...
      {
        title: "Es",
        options: ["Tanpa es", "Es sedikit", "Es normal", "Extra es"],
      },
    ],
  },
  {
    ...
    description:
      "Bubuk matcha murni Jepang diseduh dengan susu segar dingin dan pemanis alami madu.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
  ...
  // Kantin Mencintai - Dessert
  {
    ...
    description:
      "Croissant renyah mentega berlapis dihiasi irisan kacang almond renyah dan taburan gula bubuk halus.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
      {
        title: "Kondisi",
        options: ["Original", "Dipanaskan", "Dengan saus ekstra"],
      },
    ],
  },
  // Kantin Mencintai - Snack
  {
    ...
    description:
      "4 buah siomay ayam udang lembut dibalut saus mentai creamy gurih dibakar dengan keju mozzarella.",
    available: true,
    sellerId: "usr-2",
    orderOptions: [
    ...
    items: [
      {
        ...
        quantity: 1,
        image:
          "https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=150&auto=format&fit=crop&q=60",
    ...
    items: [
      {
        ...
        quantity: 2,
        image:
          "https://images.unsplash.com/photo-1562967914-608f82629710?w=150&auto=format&fit=crop&q=60",
        selectedOptions: {
          "Pilihan Nasi": "Nasi normal",
          "Level Pedas": "Extra pedas",
        },
      },
    ],
    total: 48000,
...
const INITIAL_NOTIFICATIONS: Notification[] = [
  {
    ...
    message:
      "Pesanan Chicken Rice Bowl sudah selesai, silahkan ambil di Kantin Mencintai.",
    type: "success",
    read: false,
    date: "2026-06-24T09:30:00.000Z",
    orderId: "TX-10001",
    ...
    (set, get) => ({
      ...
      login: (email, password) => {
        ...
        if (!matched) {
          return {
            success: false,
            message:
              "Akun belum terdaftar. Silakan lakukan registrasi terlebih dahulu.",
          };
        }
      ...
      register: (details) => {
        ...
        const newUser: User = {
          ...
          canteenName:
            details.role === "penjual"
              ? details.canteenName || "Kantin Saya"
              : undefined,
      ...
      updateQuantity: (cartItemId, type) =>
        set((state) => ({
      ...
      toggleCartItemChecked: (cartItemId) =>
        set((state) => ({
          cart: state.cart.map((item) =>
      ...
      toggleAllCartItemsChecked: (checked) =>
        set((state) => ({
      ...
      checkout: () => {
        ...
        if (checkedItems.length === 0) {
          return {
            success: false,
            message: "Tidak ada item yang dicentang untuk dibeli.",
          };
        }
        ...
        Object.entries(itemsBySeller).forEach(
      ...
      updateProduct: (id, updatedDetails) =>
        set((state) => ({
          products: state.products.map((p) =>
            p.id === id ? ({ ...p, ...updatedDetails } as Product) : p,
          ),
        })),
        ...
        set((state) => {
          ...
          const updatedOrders = state.orders.map((o) =>
            o.id === orderId
              ? {
                  ...o,
                  status: "Ditolak" as Order["status"],
